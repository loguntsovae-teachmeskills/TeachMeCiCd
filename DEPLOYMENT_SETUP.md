# 🚀 Инструкция по настройке автоматического деплоя на AWS EC2

## 📋 Что нужно сделать

### 1️⃣ Подготовка AWS EC2 сервера

#### На вашем EC2 сервере выполните:

```bash
# 1. Подключитесь к серверу
ssh -i your-key.pem ubuntu@your-ec2-ip

# 2. Обновите систему
sudo apt update && sudo apt upgrade -y

# 3. Установите Python и необходимые инструменты
sudo apt install -y python3 python3-pip python3-venv git curl

# 4. Создайте директорию для приложения
sudo mkdir -p /home/ubuntu/app/teachme-cicd-api
sudo chown ubuntu:ubuntu /home/ubuntu/app/teachme-cicd-api

# 5. Откройте порт 8003 в Security Group
# Это делается в AWS Console:
# EC2 → Security Groups → Ваша группа → Inbound rules → Add rule
# Type: Custom TCP, Port: 8003, Source: 0.0.0.0/0 (или ваш IP)
```

### 2️⃣ Настройка GitHub Secrets

В репозитории GitHub перейдите в: **Settings → Secrets and variables → Actions → New repository secret**

Добавьте следующие секреты:

#### `EC2_SSH_KEY`
Содержимое вашего приватного SSH ключа для доступа к EC2:

```bash
# На вашем локальном компьютере:
cat ~/.ssh/your-ec2-key.pem

# Или если у вас ключ от AWS:
cat ~/Downloads/your-ec2-key.pem
```

**Скопируйте ВСЁ содержимое** (включая `-----BEGIN RSA PRIVATE KEY-----` и `-----END RSA PRIVATE KEY-----`) и вставьте как значение секрета.

#### `EC2_HOST`
IP адрес или DNS имя вашего EC2 сервера:

```
# Примеры:
54.123.45.67
# или
ec2-54-123-45-67.compute-1.amazonaws.com
```

#### `EC2_USER`
Имя пользователя для SSH (обычно для Ubuntu AMI):

```
ubuntu
```

### 3️⃣ Проверка настроек Security Group

Убедитесь, что в Security Group вашего EC2 открыты порты:

| Type | Protocol | Port Range | Source | Description |
|------|----------|------------|--------|-------------|
| SSH | TCP | 22 | Your IP | SSH access |
| Custom TCP | TCP | 8003 | 0.0.0.0/0 | FastAPI application |

### 4️⃣ Создание develop ветки (уже создана)

```bash
# Ветка develop уже создана
# Проверить можно командой:
git branch
```

### 5️⃣ Workflow для деплоя

Файл `.github/workflows/deploy-ec2.yml` уже создан и настроен.

**Как работает деплой:**
1. При push в `main` ветку
2. Проверяется, что это merge из `develop`
3. Запускаются тесты
4. Если всё ОК - деплой на EC2
5. Проверка работоспособности

## 🔄 Процесс работы

### Обычный workflow:

```bash
# 1. Работаете в develop ветке
git checkout develop

# 2. Делаете изменения
# ... пишете код ...

# 3. Коммитите и пушите в develop
git add .
git commit -m "feat: add new feature"
git push origin develop

# 4. Создаете Pull Request из develop в main
# На GitHub: develop → main

# 5. После ревью и аппрува - мерджите PR

# 6. GitHub Actions автоматически:
#    - Запустит тесты
#    - Задеплоит на EC2
#    - Проверит работоспособность
```

### Быстрый деплой текущих изменений:

```bash
# Из develop ветки
git checkout develop
git add .
git commit -m "feat: ready for deploy"
git push origin develop

# Переключитесь на main и смержите
git checkout main
git merge develop
git push origin main

# Деплой запустится автоматически!
```

## ✅ Проверка деплоя

### После успешного деплоя проверьте:

```bash
# 1. Health check
curl http://YOUR_EC2_IP:8003/health

# Ответ:
{
  "status": "healthy",
  "timestamp": "2025-11-17T...",
  "version": "1.0.0",
  "database": "ok"
}

# 2. Deployment status (НОВЫЙ ENDPOINT!)
curl http://YOUR_EC2_IP:8003/deployment

# Ответ:
{
  "status": "deployed",
  "deployment_info": {
    "deployed_at": "2025-11-17T18:30:00Z",
    "commit_sha": "abc123...",
    "branch": "main",
    "deployed_by": "your-username",
    "workflow_run": "42"
  },
  "current_time": "2025-11-17T18:31:00",
  "server": "AWS EC2",
  "message": "Деплой успешен"
}

# 3. Swagger документация
curl http://YOUR_EC2_IP:8003/docs
# Или откройте в браузере
```

## 🐛 Troubleshooting

### Проблема: SSH ключ не работает

```bash
# Проверьте права на ключ
chmod 600 your-key.pem

# Проверьте подключение вручную
ssh -i your-key.pem ubuntu@YOUR_EC2_IP

# Если работает - проверьте, что в GitHub Secret
# скопирован именно этот ключ
```

### Проблема: Порт 8003 недоступен

```bash
# На EC2 сервере проверьте:
sudo netstat -tulpn | grep 8003

# Если процесс есть - порт занят
# Если нет - приложение не запустилось

# Посмотрите логи:
cat /home/ubuntu/app/app.log
```

### Проблема: GitHub Actions не может подключиться

1. Проверьте, что в Security Group разрешен SSH (порт 22)
2. Проверьте правильность `EC2_HOST` в secrets
3. Проверьте, что SSH ключ скопирован полностью

### Проблема: Деплой не запускается

Убедитесь, что:
- Это merge из develop в main
- Все секреты настроены
- Тесты проходят

## 📊 Мониторинг

### Просмотр логов на сервере:

```bash
# Подключитесь к EC2
ssh -i your-key.pem ubuntu@YOUR_EC2_IP

# Логи приложения
tail -f /home/ubuntu/app/app.log

# Процессы Python
ps aux | grep python

# Проверка порта
sudo netstat -tulpn | grep 8003
```

### GitHub Actions логи:

1. Откройте репозиторий на GitHub
2. Вкладка "Actions"
3. Выберите последний workflow
4. Смотрите детали каждого шага

## 🔐 Безопасность

### Рекомендации:

1. **Ограничьте SSH доступ**
   ```
   Source для SSH: только ваш IP вместо 0.0.0.0/0
   ```

2. **Используйте отдельный ключ для деплоя**
   ```bash
   # Создайте новый ключ
   ssh-keygen -t rsa -b 4096 -f ~/.ssh/deploy_key
   
   # Добавьте на сервер
   ssh-copy-id -i ~/.ssh/deploy_key.pub ubuntu@YOUR_EC2_IP
   ```

3. **Ротация ключей**
   - Меняйте SSH ключи раз в 3-6 месяцев
   - Используйте разные ключи для разных окружений

## 🎯 Чек-лист готовности

Перед первым деплоем убедитесь:

- [ ] EC2 инстанс запущен и доступен
- [ ] Python 3.11+ установлен на сервере
- [ ] Директория `/home/ubuntu/app` создана
- [ ] Security Group настроен (порты 22, 8003)
- [ ] GitHub Secrets добавлены:
  - [ ] `EC2_SSH_KEY`
  - [ ] `EC2_HOST`
  - [ ] `EC2_USER`
- [ ] SSH подключение работает вручную
- [ ] develop ветка создана
- [ ] Workflow файл `.github/workflows/deploy-ec2.yml` в репозитории

## 🚀 Первый деплой

```bash
# 1. Убедитесь что вы в develop
git checkout develop

# 2. Закоммитьте текущее состояние
git add .
git commit -m "feat: setup deployment to AWS EC2"
git push origin develop

# 3. Смержите в main
git checkout main
git merge develop
git push origin main

# 4. Следите за процессом в GitHub Actions
# GitHub → Actions → Deploy to AWS EC2

# 5. Проверьте деплой
curl http://YOUR_EC2_IP:8003/deployment
```

## 📞 Поддержка

Если что-то не работает:

1. Проверьте логи GitHub Actions
2. Подключитесь к EC2 и проверьте логи приложения
3. Проверьте все настройки по чек-листу
4. Убедитесь, что Security Group настроен правильно

---

**Готово!** После настройки каждый merge из develop в main будет автоматически деплоиться на ваш EC2 сервер! 🎉
